...
def test_postSubmission(setup_database):
    setup_database
    res = client.post('/submission', json={'submission_type':'Genre', 'submission_request':'A genre that emphasizes on coding'})
    assert res.status_code == 201
    assert res.json() == {'submission_type':'Genre', 'submission_request':'A genre that emphasizes on coding', 'id': 1}
